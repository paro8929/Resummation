...
double altget_allshiftm(int verbose)
{
  ...
  double Rag[6]={1,ITmax,max(dPi[0][0]-m02+1.e-6,0),Mmax,double(verbose),0};
...
int main(int argc, char* argv[])
{
  ...
  fstream hist;
  sprintf(buffer,"store/history-R2-NT%i-N%i-cut%i-m%f-T%f-IT%i.dat",N0,N1,int(tan(cut)),m02,Temp,mIT);
  ...
  for(int IT=1;IT<mIT;IT++)
    {
      ...
      sprintf(buffer,"finTR2-Sdump%i",IT);        
	...
	for (int j=0;j<N1;j++)
      ...
      sprintf(buffer,"finTR2-Pdump%i",IT);        
